...
# model). This is a priority/weighting policy, not a hard block â€” a claim
...
# independent LLM judgment syntheses on genuinely ambiguous claims â€” see
...
# A determinate verdict no longer settles funds immediately â€” it enters a
...
# â€” this contract has no way to hand-pick specific "witness" validators,
...
# v0.3.10 â€” validator-verified official domains: the real fix for
...
# for a protocol; VERIFICATION is not a human decision at all â€” it's a
...
# call â€” expensive, slow, and each one is an independent chance for
...
# at a flat "first 8 overall" â€” the second audit correctly flagged that as
...
# submit â€” the worst they can do is waste their OWN slots.
...
# needs a hard ceiling â€” otherwise a single malicious claim can produce an
...
# v0.3.5: no longer an LLM extraction budget (see _fetch_and_extract_facts) â€”
...
# ===========================================================================
# EVM VALUE-TRANSFER INTERFACE â€” single choke point for outbound GEN
# ===========================================================================
...
def _now_iso() -> str:
    ...
    realistically) does not change the outcome in practice â€” but this has
    ...
    replace this function's implementation, not its call sites â€” every
...
def _extract_deterministic_excerpt(normalized_text: str, subject: str, source_statement: str) -> str:
    """Deterministic, fixed-position/keyword-anchored excerpt extraction â€”
    replaces the old LLM-driven 'pick 3 relevant quotes' step (audit
    finding #6 / v0.3.5). Given the same normalized_text, this ALWAYS
    returns the same output â€” no model call, no room for two independent
    ...
    matches â€” still fully deterministic, never an LLM guess."""
...
def _content_hash(text: str) -> str:
    """Evidence Manifest content fingerprint â€” SHA-256 over the
    ...
    problem `_fetch_and_extract_facts` was rewritten to avoid â€” a dynamic
    ...
    tracked in docs/genlayer.md's audit-remediation roadmap â€” this hash is
...
# fetched server-side by the contract's own nondet web-fetch â€” an
...
# credibility ranking system (official docs vs. forum posts) â€” that needs
...
# internal colon â€” `http://[::1]/` produced host `[` instead of `[::1]`,
...
# whole contract-level filter is a floor, not a complete SSRF defense â€”
...
def _is_safe_evidence_url(url: str) -> bool:
    ...
    not complete SSRF protection â€” real network-layer egress control is
    ...
    1. Bare-decimal / non-dotted numeric hosts (e.g. "2130706433", which
       browsers/some HTTP clients resolve as 127.0.0.1) â€” no legitimate
    ...
    2. The 100.64.0.0/10 CGNAT range (RFC 6598) â€” increasingly used inside
    ...
    3. v0.3.11, caught by an adversarial test (tests/test_adversarial_inputs.py)
       ...
       URL like "http://http://127.0.0.1/" was FAILING OPEN â€” `rest` became
       ...
       rejecting outright if "://" appears ANYWHERE in `rest` â€” no
    ...
    if "://" in rest:
        return False  # doubled/nested scheme â€” see note above
    host_and_maybe_port = rest.split("/", 1)[0].split("@")[-1]
    if host_and_maybe_port.startswith("["):
        return False  # bracketed IPv6 literal â€” reject outright, see note above
    ...
    # 172.16.0.0-172.31.255.255 (private range) â€” check the second octet.
    if host.startswith("172."):
        parts = host.split(".")
        if len(parts) >= 2 and parts[1].isdigit() and 16 <= int(parts[1]) <= 31:
            return False
    # 100.64.0.0-100.127.255.255 (CGNAT, RFC 6598) â€” check the second octet.
...
def _extract_host(url: str) -> str:
    """Shared with _is_safe_evidence_url's parsing â€” pulled out so
...
def _is_verified_primary_source(url: str, official_domains: list) -> bool:
    """v0.3.7, audit remaining-blocker #2 ("source tiers are self-declared â€”
    ...
    official domains (set via set_protocol_official_domains â€” permissionless
    ...
    rejection â€” this is a weighting improvement, not a curated trusted-
...
def _normalize_domain(value: str) -> str:
    ...
    ops, no network â€” deterministic by construction."""
    v = value.strip().lower()
    if "://" in v:
        v = v.split("://", 1)[1]
    v = v.split("/", 1)[0]
...
def _extract_github_org_website(html: str) -> str:
    ...
    `<a rel="nofollow" itemprop="url" ... href="https://example.com">` â€”
    ...
    checked). Returns "" if not found â€” never raises, so a markup change on
...
class TruthStake(gl.Contract):
    ...
    def register_protocol(self, name: str, category: str) -> None:
        ...
        only â€” they never gate whether a claim can be created, so this
        registry cannot be used to censor gameplay.

        v0.3.7: the stored value is now a JSON record (category +
        official_domains), not a bare category string â€” official_domains
        ...
        set_protocol_official_domains below â€” see that method's docstring
    ...
    def set_protocol_official_domains(self, name: str, domains_csv: str) -> None:
        ...
        something beyond a self-declared enum value â€” see
        ...
        _require(str(gl.message.sender_address) == self.owner, "Only owner")
        _require(name in self.protocols, "Unknown protocol â€” register it first")
    ...
    # GenVM validator consensus, not a human â€” the exact same leader/
    ...
    # at all â€” either validators independently extract the same website
    ...
    def propose_official_domain(self, protocol_name: str, domain: str, github_org: str) -> str:
        ...
        (or user) handle whose public profile page will be checked â€”
        ...
        _require(protocol_name in self.protocols, "Unknown protocol â€” register it first")
    ...
    def verify_official_domain(self, proposal_id: str) -> None:
        """Permissionless â€” anyone can trigger verification once a proposal
        ...
        domain â€” checked with strict_eq since this whole pipeline is pure
        ...
        forfeited to the contract owner â€” not because the owner did
    ...
    def get_domain_proposal_count(self) -> int:
        # Same pattern as get_claim_count â€” proposal ids are sequential, so
    ...
    def amend_claim(self, claim_id: str, new_interpretation: str, rationale: str) -> int:
        ...
        overwritten â€” each amendment is a new immutable version."""
    ...
    def withdraw_claim(self, claim_id: str) -> None:
        ...
        challenge stake. Reward-only refund â€” nothing else was ever at
        stake at this point."""
        claim = self._get_claim_record(claim_id)
    ...
    def _record_evidence(
        ...
        identity) before reaching here â€” this is purely the record-creation
        ...
        record = {
            ...
            # judgment step actually fetches this source â€” None until then
    ...
    def submit_challenge(self, claim_id: str, argument: str) -> None:
        """One active challenge per claim at a time â€” additional challengers
        ...
        # challenged yet) â€” a challenger could race in right up to and past
    ...
    def create_bounty(
        ...
        discretionarily at resolution â€” this removes a manipulation vector
    ...
    def _refund_bounty_if_any(self, claim_id: str, sponsor_fallback: str) -> None:
        ...
        review) â€” the sponsor's funds were simply stuck in the contract
    ...
    def _pay_bounty_if_any(
        ...
        "pro-rate" it â€” but nothing ever paid it out off-chain either, so
        ...
        # human-settlement path, which never runs an AI judgment) â€” never
    ...
    def _select_judged_evidence(
        ...
        """Audit finding #3 fix â€” per-party evidence slots. See
        ...
        CORROBORATIVE_EVIDENCE_TYPES items (stable sort â€” submission order
        ...
        not two â€” VERIFIED primary (URL host matches the protocol's
        ...
        # all of their items) â€” exactly the crowd-out the per-party split
    ...
    def _run_judgment(
        ...
        the decision-relevant fields (see module docstring, Â§b).

        `guaranteed_evidence_id` (v0.3.7, audit remaining-blocker #4 â€”
        ...
        of the normal per-party slot caps â€” the whole point is that the
        ...
        for item in judged_items:
            ...
            if item.get("url"):
                ...
                # EXTRACTED facts (the equivalence-agreed value â€” identical
                ...
                # replicated record â€” so this excerpt is now genuinely
                ...
                # (the audit's stated ideal) is still out of scope â€” that
            ...
            # v0.3.7, audit remaining-blocker #2: three-value tier, not two â€”
        ...
        def validator_fn(leaders_res) -> bool:
            ...
            (see docs/genlayer.md's v0.3.3 section) â€” this check requires two
            ...
            1. `confidence` dropped from the equivalence check entirely â€” it
    ...
    def _fetch_and_extract_facts(self, url: str, subject: str, source_statement: str) -> tuple:
        """v0.3.5 rewrite â€” the deterministic-core redesign the third audit's
        ...
        downstream of the fetch â€” HTML-to-text normalization and passage
        selection â€” is now pure deterministic string processing
        ...
        checked with `strict_eq` rather than `prompt_comparative` â€” the
        ...
        The LLM is still used â€” but only once, later, in `_run_judgment`'s
        leader_fn, for the actual verdict synthesis â€” and now receives this
        ...
        on-chain isn't done) â€” but it means a full-page tampering check is
    ...
    def _build_judgment_prompt(
        ...
        return f"""
You are the semantic-judgment engine for TRUTHSTAKE, a Web3 game about
...
evidence content actually fetched below â€” never the submitter's own claims
...
- PRIMARY_UNVERIFIED: the submitter labeled it the same way (protocol
  ...
  NOT as strong as VERIFIED_PRIMARY â€” a label alone is not proof of an
  official source.
- CORROBORATIVE: forum discussions, social posts, screenshots, or other
  secondary sources. Context only â€” it may explain community sentiment or
...
a determinate verdict â€” do not let an unverified label substitute for a
...
"""
    ...
    def _apply_verdict(self, claim_id: str, judgment: dict) -> None:
        ...
        # it composed itself â€” nothing previously checked that a cited id
        ...
        # first â€” either party can call raise_appeal() to trigger a fresh,
    ...
    def _settle_claim(
        ...
        then calls `_send_gen` â€” so a re-entrant or repeated call finds a
        ...
        if verdict == VERDICT_PASSED:
            ...
            # outcome â€” a partial verdict means the challenge had merit, so
    ...
    # =======================================================================
    # APPEALS (v0.3.6, audit remaining-blocker #3 â€” appeal / independent-
    ...
    # (new leader, new validators â€” this contract has no mechanism to
    ...
    def raise_appeal(
        ...
        submit ONE new evidence item specifically for the appeal round â€”
        ...
        judged in the appeal round regardless of the normal 4-slot cap â€”
        ...
        # The appeal round can itself land on low-confidence/inconclusive â€”
        ...
        if appeal_itself_inconclusive:
            ...
            # Appeal upheld the original verdict â€” forfeit the appellant's
            ...
            # Appeal changed the outcome â€” refund the appellant's bond and
    ...
    def finalize_settlement(self, claim_id: str) -> None:
        """Permissionless, like claim_dispute_timeout â€” anyone can trigger
    ...
    def propose_human_settlement(self, claim_id: str, claimant_payout_bps: int) -> None:
        ...
        mutual-agreement path, not a unilateral one â€” neither party can
    ...
    def claim_dispute_timeout(self, claim_id: str) -> None:
        ...
        no fund can ever be permanently stuck â€” this is what makes the
