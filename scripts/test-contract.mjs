#!/usr/bin/env node
/**
 * TRUTHSTAKE contract integration test — exercises every public read/write
 ...
 * counter-argument, and real evidence URLs.
 *
 * Run: node scripts/test-contract.mjs
 * Requires: apps/web's node_modules (genlayer-js) — run from repo root
 * after `pnpm install`.
 * Override the target contract with TRUTHSTAKE_CONTRACT_ADDRESS=0x... —
 * defaults to the current deployed address.
 */
...
const CONTRACT_ADDRESS = process.env.TRUTHSTAKE_CONTRACT_ADDRESS || "0x02e8ce1Eb09D689e51c555490BfDC6CAd922C8b3";
...
async function main() {
  log("=== TRUTHSTAKE contract test — live StudioNet ===");
