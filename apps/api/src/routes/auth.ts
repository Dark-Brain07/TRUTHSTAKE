...
function buildSiweMessage(params: {
  ...
  return [
    ...
    params.address,
    "",
    "Sign in to TRUTHSTAKE. This request will not trigger a blockchain transaction or cost any gas.",
    "",
...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post("/api/v1/auth/verify", async (request, reply) => {
    ...
    // Frontend and backend are different sites, so a cross-site fetch
    // with credentials:"include" only carries the cookie back if it's
    ...
    reply.setCookie("truthstake_session", accessToken, cookieOptions);
    reply.setCookie("truthstake_refresh", refreshToken, refreshCookieOptions);
  ...
  app.post("/api/v1/auth/refresh", async (request, reply) => {
    const refreshToken = request.cookies["truthstake_refresh"];
    if (!refreshToken) {
    ...
    if (!session || session.revokedAt || session.expiresAt < new Date()) {
      reply.clearCookie("truthstake_session", { path: "/" });
      reply.clearCookie("truthstake_refresh", { path: "/api/v1/auth/refresh" });
    ...
    reply.setCookie("truthstake_session", accessToken, cookieOptions);
    reply.setCookie("truthstake_refresh", newRefreshToken, refreshCookieOptions);
