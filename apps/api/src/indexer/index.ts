 ...
 * in it — see contracts/truthstake/contract.py's `create_claim`, which never
 ...
 * events) — fine at TRUTHSTAKE's expected claim volume, and simpler than a
 ...
 */
/**
 * TIGHTENED 2026-09-05: was 15 minutes. At TRUTHSTAKE's current claim volume
...
async function main(): Promise<void> {
  ...
  const address = env.TRUTHSTAKE_CONTRACT_ADDRESS as `0x${string}`;

  // eslint-disable-next-line no-console
  console.log(`TRUTHSTAKE indexer starting — contract ${address} on studionet.`);
