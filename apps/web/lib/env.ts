...
if (!env.contractAddress) {
  ...
  console.warn(
    "NEXT_PUBLIC_TRUTHSTAKE_CONTRACT_ADDRESS is not set — contract reads/writes will fail. " +
...
export function getContractConfig(): { address: `0x${string}` } {
  if (!env.contractAddress) {
    throw new Error(
      "GenLayer contract is not configured — check NEXT_PUBLIC_TRUTHSTAKE_CONTRACT_ADDRESS",
    );
  }
