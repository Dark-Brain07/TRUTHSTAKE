# TRUTHSTAKE — GenLayer Intelligent Contract

File: [`contracts/truthstake/contract.py`](../contracts/truthstake/contract.py) — deployed at `0x02e8ce1Eb09D689e51c555490BfDC6CAd922C8b3` (GenLayer StudioNet).
...
genvm-lint check contracts/truthstake/contract.py --json   # fix any warnings before proceeding
genlayer network studionet
genlayer deploy --contract contracts/truthstake/contract.py
```
...
**Deployed** to `0x88BA19eF301C54138923B7e7F68c35F9B8469032` (2026-08-26). CACHE tables cleared, all env config (Fly secrets on `truthstake-api`/`truthstake-indexer`, Vercel `NEXT_PUBLIC_TRUTHSTAKE_CONTRACT_ADDRESS`) updated and redeployed.
...
Following up on v0.3.5's confirmed liveness fix, the user asked to address the four items flagged above ("fix all"). All four implemented in `contracts/truthstake/contract.py`:
...
All 4 test claims are real on-chain data, visible on [truthstake.vercel.app/claims](https://truthstake.vercel.app/claims) as the indexer catches up — including claim #4's `snapshot_text`/`full_page_hash` fields on its evidence, and claim #1/#2's full lifecycle (evidence, objections, bounty, challenge, judgment, settlement).
...
Scripts: `scripts/judgment-reliability-matrix.mjs` (cases 11-16) and `scripts/judgment-reliability-matrix-2.mjs` (cases 17-22), both re-runnable against any deployed address via `TRUTHSTAKE_CONTRACT_ADDRESS`.
...
- `contracts/truthstake/contract.py` line 1 still read `# v0.2.16` — a leftover `genlayer init` scaffold marker contradicting every v0.3.x claim made everywhere else. Annotated in place (not deleted — it's regenerated by tooling) to explain what it actually is.
...
Deployed the source to `0x02e8ce1Eb09D689e51c555490BfDC6CAd922C8b3` (GenLayer StudioNet). Wired the new address across all env locations (`.env.example` files, `apps/web/.env.local`, `apps/api/.env`).
