# TRUTHSTAKE — Evidence Packet

A single page pointing at reproducible proof for every claim this project makes. Nothing here is asserted without a command, file, or address a reader can check independently.

## Deployment

- **Contract address:** `0x02e8ce1Eb09D689e51c555490BfDC6CAd922C8b3` (GenLayer StudioNet)
- **Explorer:** https://studio.genlayer.com/contract/0x02e8ce1Eb09D689e51c555490BfDC6CAd922C8b3
- **Source of the deployed contract:** [`contracts/truthstake/contract.py`](../contracts/truthstake/contract.py)
...
python3 -m py_compile contracts/truthstake/contract.py
genvm-lint check contracts/truthstake/contract.py --json
...
- All fund movement flows through one function, `_send_gen`, called only from `_settle_claim`, called only from consensus-gated write methods (`submit_for_judgment`, `raise_appeal`, `finalize_settlement`, `propose_human_settlement`, `claim_dispute_timeout`). Verify: `grep -n "_send_gen\|def _settle_claim" contracts/truthstake/contract.py`.
...
- **Real, unmanaged browser wallet — manually verified (2026-09-07).** All the automated product tests (`scripts/product-test-*.mjs`) sign with `genlayer-js`'s programmatic `createAccount()`, funded via StudioNet's `sim_fundAccount` test faucet — a real on-chain signer, but not a real wallet-extension UX (no injected provider, no approval popup). That path was manually exercised end-to-end against the live app by the project owner using a real, unmanaged browser wallet: connect, every contract write (create/challenge/evidence/etc.), and transaction-status updates all confirmed working correctly. Recorded here rather than left as an unchecked chat note.
